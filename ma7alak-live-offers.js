...
function ownerBar(list){
  let lim=Math.max(0,Number(ent?.active_limit||0)),used=list.length,pct=lim?Math.min(100,used/lim*100):0,en=!!ent?.enabled;
  const activeVideo=videoLiveRows(currentOwnerSlug())[0];
  return`<div class="m7lo-owner"><div><strong>⚡ Live & Offers</strong><small>${en?`${used} / ${lim} active slots · tap EDIT or END below`:`مش مفعّل لهالمحل`}</small><div class="m7lo-slots"><i style="width:${pct}%"></i></div></div><div class="m7lo-owner-actions"><button class="m7lo-video-btn" data-m7-video-live ${!en?"disabled":""}>${activeVideo?"🔴 Resume Live":"📹 Go Live"}</button><button class="m7lo-btn" data-m7-create ${!en||used>=lim?"disabled":""}>${en&&used>=lim?"Slots Full":"＋ Add"}</button></div></div>`
}
function bind(root){
...
(function(){
  ...
  s.src="https://cdn.jsdelivr.net/gh/hadi89mok/ma7alak@89f7489aa73d3191231c349a272b1109f798c687/ma7alak-live-video.js";
  s.async=true;
