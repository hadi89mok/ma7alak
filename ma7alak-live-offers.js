...
function finalLiveOffersUxCss(){
  if($("#m7lo-final-ux-css"))return;
  const s=document.createElement("style");
  s.id="m7lo-final-ux-css";
  s.textContent=`
  .m7lo-media-count{display:flex!important;align-items:center!important;gap:5px!important;top:10px!important;right:10px!important;bottom:auto!important;padding:6px 8px!important;border-radius:999px!important;background:rgba(4,4,4,.76)!important;border:1px solid rgba(255,255,255,.22)!important;color:#fff!important;font-size:9px!important;backdrop-filter:blur(8px)!important}
  .m7lo-media-count em{font-style:normal!important;color:rgba(255,255,255,.58)!important;font-size:6.5px!important;letter-spacing:.5px!important}
  .m7lo-media-progress{left:50%!important;right:auto!important;top:14px!important;transform:translateX(-50%)!important;width:auto!important;max-width:150px!important;gap:5px!important}
  .m7lo-media-progress i{flex:0 0 7px!important;width:7px!important;height:7px!important;border-radius:50%!important;background:rgba(255,255,255,.38)!important}
  .m7lo-media-progress i.on{width:18px!important;flex-basis:18px!important;border-radius:99px!important;background:#fff!important;box-shadow:none!important}
  .m7lo-hero:after{display:none!important}
  .m7lo-view-info{border-top:1px solid rgba(255,255,255,.07)!important}
  .m7lo-view-info .m7lo-desc{max-height:3.1em!important;line-height:1.45!important;font-size:11px!important}
  .m7lo-view-info .m7lo-schedule{margin:7px 0!important;padding:7px 9px!important}
  .m7lo-view-info .m7lo-visit{margin-top:8px!important;min-height:40px!important;padding:9px!important}
  .m7lo-action-pressed{box-shadow:0 0 0 3px rgba(255,255,255,.12),0 10px 26px rgba(0,0,0,.28)!important}
  .m7lo-editor-panel,.m7lo-media-manager-panel{width:min(680px,100%)!important}
  @media(max-width:520px){
    #m7lo-overlay:not(.m7lo-shop-overlay){padding:0!important;align-items:stretch!important}
    #m7lo-overlay .m7lo-view{width:100%!important;max-height:100dvh!important;height:100dvh!important;border:0!important;border-radius:0!important}
    #m7lo-overlay .m7lo-hero{height:64dvh!important;min-height:0!important}
    #m7lo-overlay .m7lo-view-info{flex:1 1 auto!important;padding:11px 13px calc(12px + env(safe-area-inset-bottom))!important;overflow:auto!important}
    #m7lo-overlay .m7lo-view-shopline{grid-template-columns:34px minmax(0,1fr) auto!important;gap:8px!important}
    #m7lo-overlay .m7lo-view-avatar{width:34px!important;height:34px!important;border-radius:10px!important}
    #m7lo-overlay .m7lo-view-info h2{font-size:18px!important;margin:8px 0 3px!important}
    #m7lo-overlay .m7lo-close{top:max(10px,env(safe-area-inset-top))!important;right:10px!important}
    #m7lo-overlay .m7lo-gallery-pill{bottom:10px!important;right:10px!important;padding:8px 10px!important}
    #m7lo-overlay .m7lo-hero-caption{bottom:10px!important;left:10px!important;right:110px!important;font-size:9px!important;max-height:4em!important;overflow:hidden!important}
    #m7lo-overlay .m7lo-editor-panel,#m7lo-overlay .m7lo-media-manager-panel{width:100%!important;height:100dvh!important;max-height:100dvh!important;border:0!important;border-radius:0!important;padding:calc(18px + env(safe-area-inset-top)) 14px calc(20px + env(safe-area-inset-bottom))!important}
    #m7lo-overlay .m7lo-editor-panel .m7lo-panel-title,#m7lo-overlay .m7lo-media-manager-panel .m7lo-panel-title{font-size:22px!important}
    #m7lo-overlay .m7lo-editor-panel .m7lo-publish-btn{position:sticky!important;bottom:0!important;z-index:5!important;box-shadow:0 -12px 25px #090909!important}
  }`;
...
@media(max-width:520px){
  #m7lo-overlay.m7lo-editor-overlay{
    padding:0!important;
...
function membershipGateCss(){
  ...
  const s=document.createElement("style");
  s.id="m7lo-membership-gate-css";
  s.textContent="#m7lo-membership-gate{position:fixed!important;inset:0!important;z-index:2147483647!important;display:grid!important;place-items:center!important;padding:20px!important;background:rgba(0,0,0,.54)!important;backdrop-filter:blur(8px) saturate(112%)!important;-webkit-backdrop-filter:blur(8px) saturate(112%)!important;font-family:Arial,\"Segoe UI\",sans-serif!important}#m7lo-membership-gate *{box-sizing:border-box}.m7lo-gate-card{position:relative;width:min(92vw,430px);overflow:hidden;padding:24px 20px 20px;border:1px solid rgba(236,190,101,.34);border-radius:25px;background:radial-gradient(circle at 15% 0%,rgba(238,190,98,.15),transparent 34%),linear-gradient(150deg,rgba(25,21,17,.95),rgba(8,8,9,.94));box-shadow:0 28px 75px rgba(0,0,0,.52),inset 0 1px 0 rgba(255,255,255,.055);color:#fff;text-align:center}.m7lo-gate-card:before{content:\"\";position:absolute;width:180px;height:180px;right:-88px;top:-95px;border:1px solid rgba(239,193,105,.13);border-radius:50%;box-shadow:0 0 0 28px rgba(239,193,105,.025),0 0 0 57px rgba(239,193,105,.014);pointer-events:none}.m7lo-gate-close{position:absolute;z-index:2;right:12px;top:12px;width:34px;height:34px;display:grid;place-items:center;border:1px solid rgba(255,255,255,.10);border-radius:50%;background:rgba(255,255,255,.045);color:rgba(255,255,255,.72);font-size:19px;line-height:1;cursor:pointer}.m7lo-gate-icon{width:62px;height:62px;margin:2px auto 13px;display:grid;place-items:center;border:1px solid rgba(239,193,105,.30);border-radius:19px;background:linear-gradient(145deg,rgba(239,193,105,.15),rgba(239,193,105,.045));box-shadow:0 12px 30px rgba(0,0,0,.22),inset 0 1px 0 rgba(255,255,255,.05);color:#f2c66f;font-size:28px}.m7lo-gate-kicker{display:inline-flex;align-items:center;gap:6px;min-height:25px;padding:0 9px;border:1px solid rgba(239,193,105,.20);border-radius:999px;background:rgba(239,193,105,.055);color:#efc778;font-size:8px;font-weight:950;letter-spacing:.75px;text-transform:uppercase}.m7lo-gate-title{margin:13px 0 7px;color:#fff8eb;font-size:22px;line-height:1.15;font-weight:950;letter-spacing:-.45px}.m7lo-gate-copy{max-width:350px;margin:0 auto;color:rgba(255,255,255,.63);font-size:12px;line-height:1.6;font-weight:650}.m7lo-gate-plans{display:flex;justify-content:center;gap:7px;margin:15px 0 17px}.m7lo-gate-plan{min-width:92px;padding:8px 10px;border:1px solid rgba(239,193,105,.18);border-radius:12px;background:rgba(255,255,255,.035);color:#f5d595;font-size:9px;font-weight:950}.m7lo-gate-actions{display:grid;grid-template-columns:1fr;gap:8px}.m7lo-gate-upgrade{min-height:47px;display:flex;align-items:center;justify-content:center;border:0;border-radius:14px;background:linear-gradient(135deg,#f0ca78,#c98a32);color:#191108!important;text-decoration:none!important;font-size:11px;font-weight:950;box-shadow:0 10px 26px rgba(199,134,44,.20)}.m7lo-gate-later{min-height:40px;border:1px solid rgba(255,255,255,.09);border-radius:12px;background:rgba(255,255,255,.035);color:rgba(255,255,255,.68);font-size:9px;font-weight:900;cursor:pointer}.m7lo-locked-feature{opacity:.78!important;cursor:pointer!important;filter:saturate(.74)}@media(max-width:480px){#m7lo-membership-gate{padding:14px!important}.m7lo-gate-card{width:min(94vw,410px);padding:22px 17px 17px;border-radius:22px}.m7lo-gate-title{font-size:20px}.m7lo-gate-copy{font-size:11px}}";
  document.head.appendChild(s);
}
...
window.ShoufHonMembershipGate=membershipGate;
...
function scheduleTimers(){
  if(timersTimer){clearTimeout(timersTimer);timersTimer=null}
  if(document.hidden||(!items.length&&!ownerItems.length))return;
  timersTimer=setTimeout(()=>{timersTimer=null;timers();scheduleTimers()},1000);
}
  ...
  return`<div id="m7lo-panel" class="m7lo-editor-panel">
    ...
    <form id="m7lo-form" class="m7lo-form">
      ...
      <div class="m7lo-form-scroll">
        ...
        <label id="m7lo-location-wrap" class="m7lo-field" ${(type==="happening"||type==="event")?"":"hidden"}><span class="m7lo-label">LOCATION</span><input id="m7lo-location" class="m7lo-in" maxlength="120" placeholder="📍 Add location" value="${esc(x?.location_text||"")}"></label>
        <div id="m7lo-prices" class="m7lo-two m7lo-price-row" ${type==="offer"?"":"hidden"}><label class="m7lo-field"><span class="m7lo-label">ORIGINAL</span><input id="m7lo-original" class="m7lo-in" type="number" min="0" step=".01" inputmode="decimal" placeholder="$ 0.00" value="${x?.original_price??""}"></label><label class="m7lo-field"><span class="m7lo-label">OFFER PRICE</span><input id="m7lo-offer" class="m7lo-in" type="number" min="0" step=".01" inputmode="decimal" placeholder="$ 0.00" value="${x?.offer_price??""}"></label></div>
        <div id="m7lo-time-row" class="m7lo-two m7lo-time-row"><label id="m7lo-start-wrap" class="m7lo-field" ${(type==="offer"||type==="event")?"":"hidden"}><span class="m7lo-label">STARTS</span><input id="m7lo-start" class="m7lo-in" type="datetime-local"></label><label id="m7lo-finish-wrap" class="m7lo-field"><span id="m7lo-finish-label" class="m7lo-label">ENDS *</span><input id="m7lo-finish" class="m7lo-in" type="datetime-local" required></label></div>
...
function creator(){const lim=Math.max(0,Number(ent?.active_limit||0));if(!ent?.enabled||lim<=0){membershipGate("offer");return}if(ownerItems.length>=lim)return;$("#m7lo-overlay")?.remove();let d=document.createElement("div");d.id="m7lo-overlay";d.className="m7lo-editor-overlay";lock();d.innerHTML=formHtml(null);mountLiveOverlay(d);wireForm(d,null)}
function editor(id){let x=ownerItems.find(q=>String(q.id)===String(id));if(!x||String(x.shop_slug).toLowerCase()!==ownerSlug)return;$("#m7lo-overlay")?.remove();let d=document.createElement("div");d.id="m7lo-overlay";d.className="m7lo-editor-overlay";lock();d.innerHTML=formHtml(x);mountLiveOverlay(d);wireForm(d,x)}
...
async function ensureFreshLiveState(){
  ...
  liveStateRefreshPromise=(async()=>{
    ...
    if(liveStateLoadedAt&&Date.now()-liveStateLoadedAt<45000)return true;
...
(function(){
  ...
  s.src="https://cdn.jsdelivr.net/gh/hadi89mok/ma7alak@53bfd953844a43d4e189761eb2b606472b8d96cc/ma7alak-live-video.js";
  s.async=true;
