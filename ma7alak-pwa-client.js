(function(){
  ...
  function injectHead(){
    ...
    if(!document.querySelector('link[rel="apple-touch-icon"]')){
      ...
      icon.href="/pwa-icon-192.svg?v="+encodeURIComponent(VERSION);
  ...
  function css(){
    ...
    style.textContent=`
      ...
      #m7-pwa-install-card .m7p-copy b{color:#fff;font-size:13px;line-height:1.2;font-weight:900}
      ...
      #m7-pwa-install-card .m7p-install{min-height:36px;padding:0 13px;border-radius:11px;background:linear-gradient(135deg,#f4ce75,#d9a441);color:#190f07;font-size:10px;font-weight:950;box-shadow:0 7px 16px rgba(217,164,65,.16),inset 0 1px 0 rgba(255,255,255,.35)}
      ...
      @media(max-width:390px){
        #m7-pwa-install-card{grid-template-columns:42px minmax(0,1fr) auto;gap:8px;padding:9px}
        #m7-pwa-install-card .m7p-icon{width:40px;height:40px;border-radius:12px}
        #m7-pwa-install-card .m7p-copy b{font-size:12px}
        #m7-pwa-install-card .m7p-copy span{font-size:8px}
        #m7-pwa-install-card .m7p-install{min-height:34px;padding:0 10px}
      }
      @media(prefers-reduced-motion:reduce){#m7-pwa-install-card{transition:none!important}}
    `;
  ...
  function showCard(){
    ...
    if(!card){
      ...
      card.innerHTML=`
        <span class="m7p-icon" aria-hidden="true"><img src="/pwa-icon-192.svg" alt=""></span>
