...
`;
  ...
  function detailHead(s){return `${img(s.directory_options?.profile_banner_image_url||s.directory_options?.cover||s.main_image_url||s.profile_image_url,'m7d-detail-cover')}<div class="m7d-detail-head">${img(s.profile_image_url)}<div><h3>${esc(s.shop_name)}</h3><p>${esc(s.category_name||'')} · ${esc(s.location||s.area||s.city||'')}</p></div></div>`}
