  ...
  function bindEvents(){
    ...
    areaGrid.addEventListener(
      ...
      function(event){
        ...
        /* Keep Area controls visible so visitors can switch Area anytime. */
        if(areaFilterBlock){
    ...
    categoryGrid.addEventListener(
      ...
      function(event){
        ...
        /* Keep Area + Category controls visible after selection. */
        if(areaFilterBlock){
          areaFilterBlock.style.display = "block";
        }
        categorySection.classList.add(
        ...
        /*
         * Scroll only after the visitor deliberately chooses a Category.
         * Aim just above the first real shop card so its full top edge is
         * visible below the fixed Ma7alak header.
         */
        requestAnimationFrame(function(){
          requestAnimationFrame(function(){
            const firstCard =
              shopGrid.querySelector(".ma7alak-shop-card");
            ...
            const fixedHeader =
              document.getElementById("ma7alak-premium-social-header") ||
              document.getElementById("ma7alak-social-header");
    ...
    let featuredHTML = `
      ...
      <div
        class="
          ...
          ${shop.featured ? "" : "ma7alak-featured-placeholder"}
        "
        aria-hidden="${shop.featured ? "false" : "true"}"
      >
...
/* =========================================================
   DIRECTORY SCROLL GUARD — SINGLE-FILE FIX
   The global hotfix currently calls window.scrollTo(0,0) several
   times after load. On the Show Shops page only, ignore those
   unsolicited top jumps after the initial page boot. Deliberate
   category scrolling from this file still works because it uses
   smooth behavior with a non-zero target.
========================================================= */
(function(){
  "use strict";
  ...
  if(directoryPath !== "/shwf-almhlat-"){
    return;
  }

  if(window.__MA7ALAK_SHOW_SHOPS_SCROLL_GUARD__){
    return;
  }

  window.__MA7ALAK_SHOW_SHOPS_SCROLL_GUARD__ = true;
  ...
  window.scrollTo =
    function(arg1,arg2){
      ...
      /*
         Block only repeated programmatic jumps to absolute top
         during the first few seconds after directory load.
         User scrolling is untouched. Our Category scroll has a
         positive target and therefore passes normally.
      */
      if(
        Date.now() - guardStartedAt < 4500 &&
        Number.isFinite(top) &&
        top <= 0 &&
        behavior !== "smooth"
      ){
...
/* =========================================================
   SAFE UX PATCH BASED ON WORKING 8050608 BASELINE
   - Area controls stay visible after Area selection.
   - Category controls stay visible after Category selection.
   - Category click scrolls to the first rendered shop card.
   - Featured/non-featured cards reserve identical badge space.
