...
(function(){
  ...
  /*
     STORY STATE GUARD
     Profile/banner edits and Admin design previews must never manufacture
     Story activity. The ring is derived only from an active shop_stories row
     plus this browser's seen timestamp.
  */
  let latestStoryCreatedAt="";
  let latestStoryExpiresAt="";
  let storyTruthTimer=0;

  function readSeenStoryTime(){
    try{
      return new Date(localStorage.getItem("ma7alak_story_seen_"+slug)||0).getTime()||0;
    }catch(_){
      return 0;
    }
  }

  function verifiedStoryState(){
    const created=new Date(latestStoryCreatedAt||0).getTime()||0;
    const expires=new Date(latestStoryExpiresAt||0).getTime()||0;
    const hasStory=!!created&&expires>Date.now();
    return {
      hasStory,
      hasNew:hasStory&&created>readSeenStoryTime()
    };
  }

  function restoreVerifiedStoryState(clearUploadEffect){
    const wrapper=document.getElementById("ma7alak-story-wrapper");
    if(!wrapper)return;

    const truth=verifiedStoryState();
    wrapper.classList.remove("m7-story-design-preview");
    wrapper.classList.toggle("story-has-story",truth.hasStory);
    wrapper.classList.toggle("story-has-new",truth.hasNew);

    if(clearUploadEffect){
      document.getElementById("ma7alak-profile-shell")
        ?.classList.remove("m7-story-upload-active");
    }
  }

  async function refreshStoryTruth(){
    const c=await getClient();
    const currentSlug=await getSlug();
    if(!c||!currentSlug)return;

    try{
      const result=await c
        .from("shop_stories")
        .select("created_at,expires_at")
        .eq("shop_slug",currentSlug)
      ...
      latestStoryCreatedAt=String(latest?.created_at||"");
      latestStoryExpiresAt=String(latest?.expires_at||"");
      restoreVerifiedStoryState(false);
    }catch(error){
      console.warn("[ShoufHon story state guard]",error);
    }
  }

  function scheduleStoryTruth(delay=80){
    clearTimeout(storyTruthTimer);
    storyTruthTimer=setTimeout(()=>refreshStoryTruth(),delay);
  }

  function messageMatchesShop(data){
    const incoming=String(data?.shopSlug||data?.shop_slug||"").trim().toLowerCase();
    return !incoming||!slug||incoming===slug;
  }
  ...
  function installStoryTruthGuard(){
    window.addEventListener("message",event=>{
      const data=event?.data||{};
      if(!messageMatchesShop(data))return;

      if(
        data.type==="MA7ALAK_DESIGN_PREVIEW" ||
        data.type==="MA7ALAK_OWNER_PROFILE_UPDATED"
      ){
        afterCurrentEvent(()=>restoreVerifiedStoryState(true));
        if(data.type==="MA7ALAK_OWNER_PROFILE_UPDATED"){
          scheduleStoryTruth(80);
        }
        return;
      }

      if(data.type==="MA7ALAK_STORY_UPLOADED"){
        latestStoryCreatedAt=new Date().toISOString();
        latestStoryExpiresAt=new Date(Date.now()+86400000).toISOString();
        afterCurrentEvent(()=>restoreVerifiedStoryState(false));
        scheduleStoryTruth(80);
      }
    });

    window.addEventListener("ma7alak:profile-design-preview",()=>{
      afterCurrentEvent(()=>restoreVerifiedStoryState(true));
    });

    window.addEventListener("ma7alak:profile-draft-preview",()=>{
      afterCurrentEvent(()=>restoreVerifiedStoryState(true));
    });

    window.addEventListener("storage",event=>{
      if(event.key==="ma7alak_story_seen_"+slug){
        restoreVerifiedStoryState(false);
      }
    });

    document.addEventListener("visibilitychange",()=>{
      if(!document.hidden)scheduleStoryTruth(60);
    });
  }
  ...
  async function save(){
    ...
    try{
      ...
      restoreVerifiedStoryState(true);
      scheduleStoryTruth(80);
  ...
  async function start(){
    ...
    installStoryTruthGuard();
    await refreshStoryTruth();
    const c=await getClient();
    c?.auth?.onAuthStateChange?.(()=>setTimeout(refresh,80));
    window.addEventListener("ma7alak:profile-design-saved",()=>{
