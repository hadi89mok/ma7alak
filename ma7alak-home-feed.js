...
function place(){
 let r=root();if(!cfg?.enabled){r.style.display="none";return}r.style.display="block";
 r.style.setProperty("position","relative","important");
 r.style.setProperty("z-index","50","important");
 ...
 r.style.maxWidth=(cfg.max_width_px||900)+"px";
 r.style.marginTop=(28+(cfg.offset_px||0))+"px";
 let a=candidates(),i=Math.max(0,Math.min(Number(cfg.position_index??3),a.length));
 if(a[i]&&a[i].parentNode){
   a[i].parentNode.insertBefore(r,a[i]);
 }else{
...
function renderOffers(){
 ...
 box.innerHTML=offers.map(x=>{let hasMedia=!!String(x.media_url||"").trim(),media=!hasMedia?"":(x.media_type==="video"?'<video class="m7g-media" src="'+esc(x.media_url)+'" muted autoplay loop playsinline></video>':'<img class="m7g-media" src="'+esc(x.media_url)+'" alt="">'),name=x.shop_name||x.shop_slug||"Shop",avatar=x.profile_image_url?'<img src="'+esc(x.profile_image_url)+'" alt="">':'<span>'+esc(name[0]||"M")+'</span>',price=x.post_type==="offer"&&x.offer_price!=null?'<div class="m7g-price">'+(x.original_price!=null?'<span class="m7g-old">$'+esc(x.original_price)+'</span>':"")+'$'+esc(x.offer_price)+'</div>':"";return '<article class="m7g-card '+(!hasMedia?'no-media':'')+'" data-id="'+esc(x.id)+'">'+media+'<div class="m7g-copy"><span class="m7g-badge">'+({offer:"🏷️ OFFER",happening:"🟢 HAPPENING NOW",arrival:"✨ NEW ARRIVAL",event:"📅 EVENT"}[x.post_type]||"⚡ LIVE")+'</span><div class="m7g-title">'+esc(x.title)+'</div><div class="m7g-shop"><div class="m7g-avatar">'+avatar+'</div><div><div class="m7g-shopname">'+esc(name)+'</div><div class="m7g-now">● LIVE NOW</div></div></div>'+price+'<div class="m7g-times" data-start="'+esc(x.starts_at)+'" data-end="'+esc(x.ends_at)+'"></div></div></article>'}).join("");
