/* =========================================================
 MA7ALAK LIVE CHAT V7 — MOBILE PANEL + DELETE CONTROLS
 - keeps V6 avatars/unread/report/message acceptance/safe loading
 - mobile-safe dvh/safe-area panel sizing
 - delete your own individual messages
 - delete an entire conversation from inbox or open chat
 - realtime refreshes on INSERT/UPDATE/DELETE
========================================================= */
(function(){
"use strict";
if(window.__MA7ALAK_CHAT_V7__)return;window.__MA7ALAK_CHAT_V7__=true;
let client,user,mode="viewer",channel=null,settingsChannel=null;
const sleep=m=>new Promise(r=>setTimeout(r,m));
const esc=v=>String(v||"").replace(/[&<>"']/g,m=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"}[m]));
async function vr(){for(let i=0;i<100&&!window.Ma7alakAccount;i++)await sleep(50);if(!window.Ma7alakAccount)throw new Error("Ma7alak account system is not ready.");await window.Ma7alakAccount.ready();client=window.Ma7alakAccount.client;user=window.Ma7alakAccount.user;mode="viewer"}
async function or(){for(let i=0;i<100&&!window.Ma7alakOwnerAuth;i++)await sleep(50);if(!window.Ma7alakOwnerAuth)throw new Error("Ma7alak owner system is not ready.");await window.Ma7alakOwnerAuth.ready();client=window.Ma7alakOwnerAuth.client;user=window.Ma7alakOwnerAuth.user;mode="owner";return !!window.Ma7alakOwnerAuth.owner}
function css(){if(document.getElementById("m7c-v7-css"))return;document.getElementById("m7c-v6-css")?.remove();let s=document.createElement("style");s.id="m7c-v7-css";s.textContent=`
...
.m7-head-person{display:flex;align-items:center;gap:9px;min-width:0;flex:1}.m7-avatar{width:40px;height:40px;border-radius:50%;object-fit:cover;border:2px solid #d99a45;background:#30251f;flex:0 0 40px}.m7-avatar.fallback{display:grid;place-items:center;font-weight:900;color:#f1b45a}#m7-chat-title{font-weight:900;color:#f1b45a;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.m7c-icon{border:0;background:#30251f;color:#fff;border-radius:10px;padding:8px 10px;cursor:pointer;flex:0 0 auto}.m7c-danger{background:#4a211f;color:#ffb5ae}
...
@media(max-width:600px){#m7-chat-shell{padding:0;align-items:stretch;justify-content:stretch;background:#100e0d}#m7-chat{width:100%;height:100dvh;max-height:100dvh;border:0;border-radius:0;box-shadow:none}#m7-chat-head{padding-top:max(12px,env(safe-area-inset-top));min-height:calc(64px + env(safe-area-inset-top))}#m7-chat-body{padding:12px}.m7-msg{max-width:86%}}
...
function loadError(b,e){if(!b)return;console.error("Ma7alak chat load error:",e);b.innerHTML=`<div class="m7-error">Could not load this conversation.<br><small>${esc(e?.message||"Please close Messages and try again.")}</small></div>`}
