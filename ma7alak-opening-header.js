...
`;
  ...
  const HERO_IMAGE_URL = "https://6aa2c9b0ea08b9137fd5ada9.imgix.net/sandbox/55.png";
...
`;
  ...
  function sharedViewerUrl(){
    try{
      ...
      if(current?.src){
        return new URL(
          current.src.replace(/ma7alak-opening-header\.js(?:[?#].*)?$/i,"shoufhon-story-viewer.js"),
          location.href
        ).href;
      }
    }catch(_){}
    return "https://cdn.jsdelivr.net/gh/hadi89mok/ma7alak@main/shoufhon-story-viewer.js";
  }
...
function videoLiveStartedText(start){const ms=Math.max(0,Date.now()-new Date(start).getTime()),s=Math.floor(ms/1000);if(s<60)return`Started ${s}s ago`;const m=Math.floor(s/60);if(m<60)return`Started ${m}m ago`;const h=Math.floor(m/60);return`Started ${h}h ago`}
function updateLiveTimers(){liveCards.querySelectorAll(".m7-live-schedule").forEach(el=>{el.innerHTML=scheduleHTML(el.dataset.start,el.dataset.end)});liveCards.querySelectorAll("[data-video-start]").forEach(el=>{el.textContent=videoLiveStartedText(el.dataset.videoStart)})}
function getLiveSignature(list){return JSON.stringify(list.map(x=>[x.id,x.stream_id,x.is_video_live,x.post_type,x.title,x.shop_slug,x.shop_name,x.profile_image_url,x.media_url,x.media_type,x.original_price,x.offer_price,x.starts_at,x.ends_at,x.status]))}
const liveMeta=t=>({offer:"🏷️ OFFER",happening:"🟢 HAPPENING NOW",arrival:"✨ NEW ARRIVAL",event:"📅 EVENT",video_live:"🔴 VIDEO LIVE"}[t]||"⚡ LIVE");
...
function renderLive(){
  ...
  liveSub.textContent=videoCount
    ? (n===1?"🔴 1 shop is video live now":`${n} live updates · ${videoCount} video live`)
    : (n===1?"1 update happening now":`${n} updates — swipe to see more`);
  liveCards.innerHTML=livePosts.map(x=>{
    const isVideo=!!(x?.is_video_live||x?.post_type==="video_live");
    let price=x.post_type==="offer"&&(x.original_price!=null||x.offer_price!=null)?`<div class="m7-live-price">${x.original_price!=null?`<span class="m7-live-old">${esc(x.original_price)}</span>`:""}${x.offer_price!=null?`${esc(x.offer_price)}`:""}</div>`:"";
    return`<article class="m7-live-card ${isVideo?"m7-video-live-card":""}" data-live-id="${esc(x.id)}" ${isVideo?`data-live-video="${esc(x.stream_id||String(x.id||"").replace(/^video:/,""))}"`:""}>${liveMedia(x)}<div class="m7-live-copy"><span class="m7-live-badge">${liveMeta(x.post_type)}</span><div class="m7-live-title">${esc(x.title)}</div><div class="m7-shop-line">${liveAvatar(x)}<div class="m7-shop-info"><div class="m7-shop-name">${esc(x.shop_name||x.shop_slug)}</div><div class="m7-shop-now">${isVideo?"● BROADCASTING NOW":"● LIVE NOW"}</div></div></div>${price}<div class="m7-live-times">${isVideo?`<div class="m7-video-started" data-video-start="${esc(x.starts_at)}">${videoLiveStartedText(x.starts_at)}</div>`:`<div class="m7-live-schedule" data-start="${esc(x.starts_at)}" data-end="${esc(x.ends_at)}"></div>`}</div></div></article>`;
  }).join("");
  liveCards.querySelectorAll("[data-live-id]").forEach(card=>{
    card.onclick=()=>{
      const streamId=card.dataset.liveVideo;
      if(streamId)return window.ShoufHonLiveVideo?.open?.(streamId);
      window.postMessage({type:"MA7ALAK_LIVE_OFFERS_VIEW",id:card.dataset.liveId},"*");
    };
  });
  updateLiveTimers();
}
...
function mergeHomepageVideoLive(next){
  const normal=(Array.isArray(next)?next:[]).filter(x=>!(x?.is_video_live||x?.post_type==="video_live"));
  let video=[];
  try{video=Array.isArray(window.ShoufHonLiveVideo?.bridgeItems)?window.ShoufHonLiveVideo.bridgeItems:[]}catch(_){video=[]}
  return [...video,...normal];
}
function applyDirectLiveState(next){
  next=mergeHomepageVideoLive(next);
...
window.addEventListener("shoufhon:live-video-state",syncLiveFromProviderCache);
...
function renderSpotlight(){if(!spotlightRoot)return;const cards=spotlightRows.map(s=>{const o=s.directory_options||{},featured=spotlightFeatured(s),cover=spotlightSafeUrl(o.profile_banner_image_url||s.profile_image_url),avatar=spotlightSafeUrl(s.profile_image_url),accent=/^#[0-9a-f]{6}$/i.test(o.home_spotlight_accent||"")?o.home_spotlight_accent:(o.story_color||"#e3b85f"),label=o.home_spotlight_label||(featured?"FEATURED":"NEW"),about=o.home_spotlight_about||s.about_text||`Discover ${s.shop_name} on ShoufHon.`,locationText=String(s.location||"").trim()||"Location coming soon",workType=String(s.category_name||"").trim()||"Local business";return `<article class="m7hs-card ${featured?"featured":""}" data-slug="${esc(s.shop_slug)}" style="--accent:${esc(accent)}" tabindex="0" role="link">${cover?`<img class="m7hs-cover" src="${esc(cover)}" alt="" loading="lazy" decoding="async">`:""}<span class="m7hs-ribbon">${esc(label)}</span><div class="m7hs-body"><div class="m7hs-profile">${avatar?`<img src="${esc(avatar)}" alt="" loading="lazy" decoding="async">`:'<span class="m7hs-fallback">✦</span>'}<div><strong>${esc(s.shop_name)}</strong><small>${esc(workType)}</small></div></div><p class="m7hs-about">${esc(about)}</p><div class="m7hs-bottom"><span class="m7hs-location">⌖ ${esc(locationText)}</span><button class="m7hs-button" type="button">${esc(o.home_spotlight_cta||"Discover shop")} →</button></div></div></article>`}).join("");spotlightRoot.innerHTML=`<header class="m7hs-head"><div><div class="m7hs-kicker">SHOUFHON SPOTLIGHT</div><h2>Local spotlight</h2></div><span class="m7hs-count">${spotlightRows.length} ${spotlightRows.length===1?"shop":"shops"} · swipe</span></header>${cards?`<div class="m7hs-track">${cards}</div>`:'<div class="m7hs-empty"><strong>Fresh local picks are coming</strong>Featured and new shops will appear here automatically.</div>'}`;spotlightRoot.querySelectorAll("[data-slug]").forEach((card,i)=>{card.onclick=e=>{e.preventDefault();spotlightGo(spotlightRows[i])};card.onkeydown=e=>{if(e.key==="Enter"||e.key===" "){e.preventDefault();spotlightGo(spotlightRows[i])}}});spotlightBindAuto();spotlightScheduleAuto(4000)}
async function loadSpotlight(){if(!spotlightRoot)return;const r=await sb.from("shop_profiles").select("shop_slug,shop_name,shop_url,profile_image_url,category_name,location,about_text,featured,is_active,directory_options").eq("is_active",true);if(r.error){console.warn("Homepage spotlight:",r.error);return}let next=(r.data||[]).filter(s=>spotlightFeatured(s)||String(s.directory_options?.badge||"").toLowerCase()==="new"),signature=JSON.stringify(next.map(s=>[s.shop_slug,s.shop_name,s.shop_url,s.profile_image_url,s.category_name,s.location,s.about_text,s.featured,s.directory_options?.profile_banner_image_url,s.directory_options?.home_spotlight_label,s.directory_options?.home_spotlight_about,s.directory_options?.home_spotlight_accent,s.directory_options?.home_spotlight_cta,s.directory_options?.badge,s.directory_options?.featured_until]));if(signature===spotlightSignature&&spotlightRows.length===next.length)return;spotlightSignature=signature;spotlightRows=spotlightShuffle(next.slice());renderSpotlight()}
loadSpotlight();
