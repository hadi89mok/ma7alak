...
var editorOpen=false,productOpen=false,productIndex=0,productItems=[],editorView={mode:"home",sectionId:null,itemId:null};
...
async function refresh(){
  try{snap=await publicLoad();theme(snap);renderPublic()}catch(e){console.warn("[ShoufHon Catalog]",e);collapse()}
}
...
function openProduct(id){
  ...
  productOpen=true;renderProduct();$("#cat-product").classList.add("open");$("#cat-product").setAttribute("aria-hidden","false");portal(true,"catalog-product-viewer");
}
function closeProduct(opt){
  ...
  if(!opt?.fromPortalBack)portal(false,"catalog-product-viewer");
}
...
window.addEventListener("message",function(event){
  ...
  if(d.type==="MA7ALAK_OWNER_STATE"){
    ...
    isOwner=!!d.isOwner;renderPublic();return;
  }
  ...
  if(d.type==="SHOUFHON_CATALOG_INVALIDATE"){
    ...
    if(editorOpen){request("load").then(function(x){ownerSnap=x;snap=x;theme(x);renderEditor()}).catch(function(){})}else refresh();
    return;
  }
...
async function start(){
  root=$("#"+ROOT_ID);if(!root)return;inject();heightObserver();
  var s=await resolveSlug();if(!s){collapse();return}
  root.setAttribute("data-shop-slug",s);registerRealtime();ownerState();setTimeout(ownerState,400);setTimeout(ownerState,1200);await refresh();
}
addEventListener("pagehide",function(){unregisterRealtime();if(editorOpen){editorBusy=false;closeEditor({fromPortalBack:true})}if(productOpen)closeProduct({fromPortalBack:true})});
start().catch(function(e){console.warn("[ShoufHon Catalog start]",e);collapse()});
})();