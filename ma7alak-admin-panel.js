...
let sb=null,userRows=[],presenceMap=new Map(),analyticsTimer=null;
...
async function loadOwnerState(){let slug=document.getElementById("m7adm-owner-shop")?.value,state=document.getElementById("m7adm-owner-state"),email=document.getElementById("m7adm-owner-email");if(!slug){state.className="m7adm-ownerstate bad";state.textContent="❌ Select a shop to check its owner.";email.value="";return}state.className="m7adm-ownerstate";state.textContent="Checking account…";let r=await sb.rpc("ma7alak_admin_owner_for_shop",{p_shop_slug:slug});if(r.error){state.className="m7adm-ownerstate bad";state.textContent="❌ "+r.error.message;return}let d=Array.isArray(r.data)?r.data[0]:r.data;if(d&&(d.email||d.user_email||d.user_id)){let e=d.email||d.user_email||"Assigned Google account";email.value=e.includes("@")?e:"";state.className="m7adm-ownerstate good";state.textContent="✅ Account found and assigned: "+e}else{email.value="";state.className="m7adm-ownerstate bad";state.textContent="❌ No email assigned to this shop yet."}}
async function assignOwner(){let slug=document.getElementById("m7adm-owner-shop")?.value,email=document.getElementById("m7adm-owner-email")?.value.trim(),state=document.getElementById("m7adm-owner-state");if(!slug||!email){state.className="m7adm-ownerstate bad";state.textContent="❌ Select a shop and enter the existing Google account email.";return}state.className="m7adm-ownerstate";state.textContent="Assigning…";let r=await sb.rpc("ma7alak_admin_assign_shop_owner",{p_shop_slug:slug,p_email:email});if(r.error){state.className="m7adm-ownerstate bad";state.textContent="❌ "+r.error.message;return}await loadOwnerState()}
async function removeOwner(){let slug=document.getElementById("m7adm-owner-shop")?.value;if(!slug)return alert("Select a shop first.");if(!confirm("Remove this owner's access from the selected shop? The normal Google account and shop will remain."))return;let r=await sb.rpc("ma7alak_admin_remove_shop_owner",{p_shop_slug:slug});if(r.error)alert(r.error.message);else loadOwnerState()}
