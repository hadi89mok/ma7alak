    ...
    */
    ...
    try{
      ...
      */
      ...
      if(!savedProfile.data)throw new Error("The shop was not found after saving.");

      const savedOptions =
        savedProfile.data.directory_options &&
        typeof savedProfile.data.directory_options === "object"
          ? savedProfile.data.directory_options
          : {};
...
function renderUsers(){let box=document.getElementById("m7adm-user-list");if(!box)return;let q=(document.getElementById("m7adm-user-search")?.value||"").trim().toLowerCase();let rows=userRows.filter(u=>!q||[u.display_name,u.username,u.email,u.account_status,u.owner_shop_name,u.owner_shop_slug,u.is_admin?"admin":""].some(v=>String(v||"").toLowerCase().includes(q)));box.innerHTML=rows.length?rows.map(u=>{let p=presenceMap.get(String(u.user_id)),on=!!p?.is_online,adminTag=u.is_admin===true?'<span class="m7adm-admin-tag">ADMIN</span>':"",ownerLine=u.owner_shop_slug?`<small style="color:#efc66e;font-weight:800">🏪 Assigned shop: ${esc(u.owner_shop_name||u.owner_shop_slug)} · /${esc(u.owner_shop_slug)}</small>`:"";return `<div class="m7adm-user ${u.account_status!=="active"?"m7adm-banned":""}" data-user-row="${esc(u.user_id)}">${u.avatar_url?`<img class="m7adm-avatar" src="${esc(u.avatar_url)}">`:'<div class="m7adm-avatar"></div>'}<div class="m7adm-copy"><strong>${esc(u.display_name||u.username||"Google User")}${adminTag}</strong><small class="m7adm-email">${esc(u.email||"Email unavailable")}</small>${ownerLine}<small><span class="m7adm-presence"><i class="m7adm-dot ${on?"on":"off"}"></i>${on?"Browsing now":"Offline"}</span> · Account: ${esc(u.account_status||"active")}</small><small>Created: ${esc(u.created_at?new Date(u.created_at).toLocaleString():"")}</small>${u.banned_reason?`<small>Reason: ${esc(u.banned_reason)}</small>`:""}</div><div class="m7adm-actions">${u.account_status==="banned"?`<button class="m7adm-btn ok" data-act="active" data-id="${esc(u.user_id)}">Unban</button>`:`<button class="m7adm-btn ban" data-act="banned" data-id="${esc(u.user_id)}">Ban</button>`}<button class="m7adm-btn del" data-act="deleted" data-id="${esc(u.user_id)}">Delete permanently</button></div></div>`}).join(""):'<div class="m7adm-empty">No matching viewer users.</div>';box.querySelectorAll("[data-act]").forEach(b=>b.onclick=async()=>{const action=b.dataset.act,id=b.dataset.id;if(action==="deleted"){let target=userRows.find(u=>String(u.user_id)===String(id)),shopText=target?.owner_shop_slug?`\n\nAssigned shop access: ${target.owner_shop_name||target.owner_shop_slug} (/${target.owner_shop_slug}) will also be removed. The shop itself stays.`:"";if(!confirm("Permanently delete this Google user and all linked ShoufHon account data? This cannot be undone."+shopText))return;b.disabled=true;b.textContent="Deleting…";const r=await sb.functions.invoke("delete-ma7alak-user",{body:{user_id:id}});if(r.error||r.data?.success===false){b.disabled=false;b.textContent="Delete permanently";alert(r.data?.error||r.error?.message||"Could not delete user.");return}userRows=userRows.filter(u=>String(u.user_id)!==String(id));document.querySelector(`[data-user-row="${CSS.escape(id)}"]`)?.remove();await Promise.all([loadUsers(),loadOwnerState()]);return}let reason=action==="banned"?prompt("Ban reason:","Chat abuse"):null;let r=await sb.rpc("ma7alak_admin_set_user_status",{p_user_id:id,p_status:action,p_reason:reason});if(r.error)alert(r.error.message);else loadUsers()})}
async function loadUsers(){try{let [u,,assignments,shops,admins]=await Promise.all([sb.rpc("ma7alak_admin_list_users"),loadPresence(),sb.rpc("ma7alak_admin_owner_assignments"),sb.from("shop_profiles").select("shop_slug,shop_name"),sb.rpc("ma7alak_admin_site_admin_ids")]);if(u.error)throw u.error;if(assignments.error)throw assignments.error;if(shops.error)throw shops.error;if(admins.error)throw admins.error;let shopNames=new Map((shops.data||[]).map(x=>[String(x.shop_slug||"").toLowerCase(),x.shop_name||x.shop_slug])),ownerByUser=new Map((assignments.data||[]).map(x=>[String(x.user_id),{slug:String(x.shop_slug||""),name:shopNames.get(String(x.shop_slug||"").toLowerCase())||x.shop_slug||""}])),adminIds=new Set((admins.data||[]).map(x=>String(x.user_id||"")).filter(Boolean));userRows=(u.data||[]).map(row=>{let owner=ownerByUser.get(String(row.user_id));return {...row,owner_shop_slug:owner?.slug||"",owner_shop_name:owner?.name||"",is_admin:adminIds.has(String(row.user_id))}});renderUsers()}catch(e){let b=document.getElementById("m7adm-user-list");if(b)b.innerHTML=`<div class="m7adm-empty">${esc(e.message||"Could not load users.")}</div>`}}
...
(function(){
  ...
  function css(){
    ...
    style.textContent = `
      ...
      .m7ds-vip-master{grid-column:1/-1;display:flex;align-items:center;justify-content:space-between;gap:14px;min-height:58px;padding:10px 12px;border:1px solid rgba(216,170,88,.24);border-radius:13px;background:linear-gradient(135deg,rgba(216,170,88,.09),rgba(255,255,255,.018));cursor:pointer}
      .m7ds-vip-master-copy{min-width:0}.m7ds-vip-master-copy b{display:block;color:#f3e2c2;font-size:10px}.m7ds-vip-master-copy small{display:block;margin-top:3px;color:#948875;font-size:8px}
      .m7ds-vip-master>input{position:absolute;width:1px;height:1px;opacity:0;pointer-events:none}
      .m7ds-vip-switch{position:relative;flex:0 0 82px;height:34px;border:1px solid #ffffff1c;border-radius:999px;background:#24211e;box-shadow:inset 0 2px 8px #0008;transition:.2s}
      .m7ds-vip-switch i{position:absolute;top:4px;left:4px;width:24px;height:24px;border-radius:50%;background:#77716a;box-shadow:0 3px 8px #0008;transition:.2s}
      .m7ds-vip-switch b::after{content:"OFF";position:absolute;top:50%;right:11px;transform:translateY(-50%);color:#aaa198;font-size:9px;letter-spacing:.8px}
      .m7ds-vip-master>input:checked+.m7ds-vip-switch{border-color:#e4b65788;background:linear-gradient(135deg,#6f4c16,#33240e);box-shadow:0 0 18px #d9a4412e,inset 0 2px 8px #0005}
      .m7ds-vip-master>input:checked+.m7ds-vip-switch i{left:52px;background:linear-gradient(135deg,#fff0af,#d9a441);box-shadow:0 0 12px #e3b64e88}
      .m7ds-vip-master>input:checked+.m7ds-vip-switch b::after{content:"ON";left:13px;right:auto;color:#ffe59c}
      .m7ds-vip-master>input:focus-visible+.m7ds-vip-switch{outline:2px solid #f0c46a;outline-offset:3px}
...
(function(){
  ...
  const DEFAULTS={
    enabled:true,background_url:"builtin",background_overlay:50,background_position:"center 54%",background_zoom:105,background_color:"#080604",accent_color:"#d9a441",accent_color_2:"#f2cc7b",panel_color:"#0b0805",text_color:"#f7dfaa",muted_color:"#d8c8a8",logo_url:"https://6aa2c9b0ea08b9137fd5ada9.imgix.net/sandbox/hadi%20new.png",logo_size:150,logo_animation:"float-glow",title:"شوف هون المحلات",title_color:"#e7bd6b",title_size:31,title_animation:"glow",subtitle:"اكتشف محلات منطقتك بمكان واحد",subtitle_color:"#e8d5b5",subtitle_size:17,subtitle_animation:"fade",top_line_enabled:true,top_line_color:"#d9a441",top_line_animation:"travel",nav_color:"#f4d694",nav_animation:"none",navigation:[{label:"الرئيسية",url:"/",enabled:true},{label:"شوف المحلات",url:"/shwf-almhlat-",enabled:true},{label:"ضيف محلك",url:"/add-shop-",enabled:true}],social_icon_color:"#f1c86e",social_glow_color:"#d9a441",social_size:46,social_animation:"float",socials:[{type:"instagram",label:"Instagram",url:"",custom_icon_url:"",enabled:false},{type:"tiktok",label:"TikTok",url:"",custom_icon_url:"",enabled:false},{type:"whatsapp",label:"WhatsApp",url:"",custom_icon_url:"",enabled:false}],copyright_text:"© 2026 ShoufHon. All rights reserved.",copyright_color:"#f0e4d0",copyright_size:13,copyright_animation:"none",bottom_text:"Made with Love",bottom_color:"#f4d694",bottom_size:14,bottom_animation:"heartbeat",show_flag:true,footer_min_height:360,top_radius:34,content_max_width:760,overall_animation:"subtle",animation_speed:100,animation_intensity:65,cedar_enabled:true,side_note_enabled:true,side_note_text:"Lebanon\nLocal\nAlways ♡",side_note_color:"#cfa660"
  };
...
`;(document.head||document.documentElement).appendChild(s)
      ...
      <div class="m7hfa-main">
        <div class="m7hfa-form">
          ...
          <div class="m7hfa-pane" data-pane="design">
            ...
            <div class="m7hfa-card"><h3>Logo</h3><div class="m7hfa-grid">${field("Logo URL","m7hfa-logo-url","url","",true)}${field("Logo size","m7hfa-logo-size","range",'min="60" max="240" step="2"')}<label class="m7hfa-field"><span>Upload logo</span><input id="m7hfa-logo-file" type="file" accept="image/png,image/jpeg,image/webp,image/gif"></label></div><button class="m7hfa-btn" type="button" data-upload="logo">Upload Logo</button></div>
          </div>
  ...
  function renderLists(){
    ...
    social.innerHTML=(settings.socials||[]).map((x,i)=>`
      <div class="m7hfa-social-row" data-social-row="${i}">
        <input type="checkbox" data-k="enabled" ${x.enabled===true?"checked":""} aria-label="Enable">
        <label><span>Platform</span><select data-k="type">${SOCIAL_TYPES.map(t=>`<option value="${t}" ${t===x.type?"selected":""}>${t}</option>`).join("")}</select></label>
        <label><span>Label</span><input type="text" data-k="label" value="${esc(x.label||"")}" placeholder="Label"></label>
        <button class="m7hfa-social-delete" type="button" data-del-social="${i}" aria-label="Delete">×</button>
        <label class="m7hfa-social-link"><span>Link / WhatsApp number</span><input type="text" data-k="url" value="${esc(x.url||"")}" placeholder="Link or WhatsApp number"></label>
        <label class="m7hfa-social-icon-url"><span>Custom icon image URL (optional)</span><input type="url" data-k="custom_icon_url" value="${esc(x.custom_icon_url||"")}" placeholder="https://..."></label>
        <label class="m7hfa-social-file"><span>Or upload custom icon</span><input type="file" data-social-icon-file accept="image/png,image/jpeg,image/webp,image/gif"></label>
        <button class="m7hfa-social-upload" type="button" data-upload-social="${i}">Upload icon</button>
      </div>
    `).join("");
  }
  ...
  function renderPreview(){
    ...
    const social=(s.socials||[]).filter(x=>x.enabled===true&&x.url).map(x=>{const type=String(x.type||"website").toLowerCase();const custom=String(x.custom_icon_url||"").trim();const art=/^https?:\/\//i.test(custom)?`<img src="${esc(custom)}" alt="">`:(FOOTER_SOCIAL_ICONS[type]||FOOTER_SOCIAL_ICONS.website);return `<b title="${esc(x.label||type)}">${art}</b>`}).join("");
  ...
  async function uploadSocialIcon(index){
    const row=$('[data-social-row="'+index+'"]',overlay);
    if(!row)return;

    const input=$("[data-social-icon-file]",row);
    const file=input?.files?.[0];

    if(!file){
      setStatus("Choose an icon image first.","error");
      return;
    }

    if(!/^image\/(jpeg|png|webp|gif)$/i.test(file.type||"")){
      setStatus("Use JPG, PNG, WEBP or GIF for the social icon.","error");
      return;
    }

    if(file.size>5*1024*1024){
      setStatus("Social icon image must be under 5 MB.","error");
      return;
    }

    const button=$("[data-upload-social]",row);
    if(button){button.disabled=true;button.textContent="Uploading…";}

    try{
      const c=await readyClient();
      const admin=await c.rpc("is_site_admin");
      if(admin.error||admin.data!==true)throw new Error("Admin session is not valid.");

      const ext=((file.name||"").split(".").pop()||"png").toLowerCase().replace(/[^a-z0-9]/g,"")||"png";
      const storagePath=`site-footer/social-icons/${Date.now()}-${Math.random().toString(36).slice(2,8)}.${ext}`;
      const up=await c.storage.from(BUCKET).upload(storagePath,file,{cacheControl:"31536000",upsert:false,contentType:file.type||undefined});
      if(up.error)throw up.error;

      const pub=c.storage.from(BUCKET).getPublicUrl(storagePath);
      const url=pub?.data?.publicUrl||"";
      if(!url)throw new Error("Could not create public icon URL.");

      const urlInput=$("[data-k=custom_icon_url]",row);
      if(urlInput)urlInput.value=url;

      collect();
      markDirty();
      renderPreview();
      setStatus("Custom social icon uploaded. Save Changes to publish.","success");
    }catch(e){
      setStatus(e?.message||"Social icon upload failed.","error");
    }finally{
      if(button){button.disabled=false;button.textContent="Upload icon";}
    }
  }
