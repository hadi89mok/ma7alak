  ...
  (function ma7alakAdminV2(){
    ...
    function installRealtime(){
      ...
      v2Realtime=supabaseClient.channel("ma7alak-admin-directory-v2")
        ...
        .on("postgres_changes",{event:"*",schema:"public",table:"shop_stories"},async()=>{await loadManagedShops();})
        .subscribe();
    }
...
function planHtml(p,i){return `<article class="m7sa-plan" data-plan-index="${i}"><div class="m7sa-plan-head"><strong>Plan ${i+1} · ${esc(p.name||"Untitled")}</strong><div class="m7sa-actions"><button class="m7sa-btn" type="button" data-plan-up>↑</button><button class="m7sa-btn" type="button" data-plan-down>↓</button><button class="m7sa-btn danger" type="button" data-plan-remove>Delete</button></div></div><div class="m7sa-grid">${field("", "Plan ID",p.id).replace('id=""','data-p="id"')}${field("","Name",p.name).replace('id=""','data-p="name"')}${field("","Price",p.price).replace('id=""','data-p="price"')}${field("","Currency",p.currency||"$").replace('id=""','data-p="currency"')}${field("","Billing period",p.period||"month").replace('id=""','data-p="period"')}${field("","Plan icon — emoji or image URL",p.icon||"").replace('id=""','data-p="icon"')}${field("","Badge text",p.badge||"").replace('id=""','data-p="badge"')}${field("","Accent color",p.accent||"#e5b75f","color").replace('id=""','data-p="accent"')}${field("","Button label",p.cta_label||"").replace('id=""','data-p="cta_label"')}${field("","Button URL or #section",p.cta_url||"").replace('id=""','data-p="cta_url"')}${field("","Description",p.description||"","textarea",true).replace('id=""','data-p="description"')}${selectField("Frame / card animation","animation_style",p.animation_style||"gold-flow",[["gold-flow","Gold moving frame"],["breathe","Breathing glow"],["shimmer","Light shimmer"],["snake","Snake light frame"],["comet","Comet frame"],["neon","Neon frame"],["electric","Electric frame"],["fire","Fire / amber frame"],["diamond","Diamond sparkle"],["rainbow-flow","Rainbow flow"],["aurora","Aurora background"],["vip-luxury","VIP Luxury — everything"],["vip-energy","VIP Energy — everything"]])}${selectField("Glow / animation strength","animation_strength",p.animation_strength||"strong",[["low","Low"],["medium","Medium"],["strong","Strong"],["very-strong","Even stronger"],["extreme","MAX / Extreme"]])}${selectField("Icon animation","icon_animation",p.icon_animation||"none",[["none","Off"],["float","Float"],["pulse","Pulse"],["spin","Spin"],["orbit","Orbit"],["bounce","Bounce"],["glow","Glow"]])}${selectField("Plan title animation","text_animation",p.text_animation||"none",[["none","Off"],["glow","Glow"],["neon","Neon"],["float","Float"],["shimmer","Shimmer"],["pulse","Pulse"]])}${selectField("Price animation","price_animation",p.price_animation||"none",[["none","Off"],["glow","Glow"],["pulse","Pulse"],["shimmer","Shimmer"]])}${selectField("Button animation","button_animation",p.button_animation||"none",[["none","Off"],["shine","Light sweep"],["energy","Energy glow"],["pulse","Pulse"],["glow","Glow"]])}${selectField("Badge animation","badge_animation",p.badge_animation||"none",[["none","Off"],["pulse","Pulse"],["bounce","Bounce"],["glow","Glow"],["shimmer","Shimmer"]])}</div><label class="m7sa-check"><input type="checkbox" data-p="active" ${p.active!==false?"checked":""}> Show this plan</label><label class="m7sa-check"><input type="checkbox" data-p="featured" ${p.featured?"checked":""}> Highlight as recommended</label><label class="m7sa-check"><input type="checkbox" data-p="animation_enabled" ${p.animation_enabled?"checked":""}> Animation ON / OFF</label><div class="m7sa-features">${(p.features||[]).map(featureHtml).join("")}</div><button class="m7sa-btn" type="button" data-feature-add>＋ Add benefit</button></article>`}
...
let sb=null,userRows=[],presenceMap=new Map(),analyticsTimer=null,currentOwnerUserId=null,adminRealtime=null;
...
if(location.pathname.replace(/\/+$/,'')!=='/admin')return;
const esc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
const shopFields=[['cover','Directory cover photo URL','url'],['story_color','Story ring color (matches this shop)','color'],['card_color','Shop card accent / border color','color'],['card_glow','Animate this card color with a glow','checkbox'],['price','Price level ($, $$, $$$)','text'],['badge','Card badge (e.g. NEW; leave blank for automatic)','text'],['order','Display order (lower comes first)','number'],['latitude','Latitude — enables Near me','number'],['longitude','Longitude — enables Near me','number'],['rating','Verified rating, 0–5 (optional)','number'],['review_count','Actual number of reviews (optional)','number']];
...
function mountForm(form,prefix){if(!form||form.querySelector('.m7da-fields'))return;const box=document.createElement('fieldset');box.className='m7da-fields';box.innerHTML='<legend>Show Shops — card & on-page details</legend><p>Your logo stays separate from the wide cover photo. Leave ratings blank unless you have a real review source.</p><div class="m7da-grid">'+shopFields.concat(profileFields).map(([k,l,t])=>field(prefix+k,l,t)).join('')+'</div>';form.append(box);const home=document.createElement('fieldset');home.className='m7da-fields m7da-home-fields';home.innerHTML='<legend>Homepage spotlight</legend><p>These optional fields control this shop’s creative Featured / New homepage card. Featured and New status still come from the shop controls above.</p><div class="m7da-grid">'+homeFields.map(([k,l,t])=>field(prefix+k,l,t)).join('')+'</div>';form.append(home);const cover=box.querySelector('#'+prefix+'cover');const upload=document.createElement('input');upload.type='file';upload.accept='image/jpeg,image/png,image/webp';const label=document.createElement('label');label.className='m7da-field';label.textContent='Or upload a cover photo (up to 8 MB)';label.append(upload);cover.parentElement.after(label);upload.onchange=async()=>{const f=upload.files?.[0];if(!f)return;if(!/^image\/(jpeg|png|webp)$/.test(f.type)||f.size>8*1024*1024){alert('Choose a JPG, PNG or WebP image up to 8 MB.');return}upload.disabled=true;const c=window.Ma7alakAdminClient;const path='directory/'+crypto.randomUUID()+'.'+({"image/jpeg":'jpg',"image/png":'png',"image/webp":'webp'}[f.type]);try{const r=await c.storage.from('shop-gallery').upload(path,f,{cacheControl:'86400',upsert:false,contentType:f.type});if(r.error)throw r.error;cover.value=c.storage.from('shop-gallery').getPublicUrl(path).data.publicUrl;label.firstChild.textContent='Uploaded. Save the shop to apply this cover.'}catch(e){alert(e.message||'Upload failed.')}finally{upload.disabled=false}}}
window.Ma7alakDirectoryAdmin={
 fill(shop){mount();const form=document.getElementById('ma-admin-edit-form');form.dataset.directoryOptions=JSON.stringify(shop.directory_options||{});shopFields.concat(homeFields).forEach(([k,l,t])=>{const input=document.getElementById('m7de-'+k),value=shop.directory_options?.[k];if(t==='checkbox')input.checked=value===true||String(value).toLowerCase()==='true';else input.value=value??(t==='color'?'#e3b85f':'')});profileFields.forEach(([k])=>document.getElementById('m7de-'+k).value=shop[k]??'')},
