...
function preflight(){
  if($("#m7lv-preflight"))return;const p=document.createElement("div");p.id="m7lv-preflight";const shop=window.Ma7alakOwnerAuth?.shop||{};
...
async function setupRoom(nextRole){
  ...
  roomChannel.on("presence",{event:"sync"},updateCount).on("broadcast",{event:"chat"},({payload})=>addMessage(payload?.name,payload?.text,payload?.sender===roomClientKey)).on("broadcast",{event:"heart"},()=>spawnHeart()).on("broadcast",{event:"live-ended"},()=>{if(mode==="audience"){setStatus("Live ended.");setTimeout(()=>cleanupOverlay(false),1800)}}).subscribe(async status=>{if(status==="SUBSCRIBED"){try{await roomChannel.track({role:nextRole,joined_at:new Date().toISOString()})}catch(_){}updateCount()}});
}
...
function addMessage(name,text,self){if(!overlay||!text)return;const list=$("#m7lv-chat-list",overlay);if(!list)return;const d=document.createElement("div");d.className="m7lv-msg"+(self?" me":"");const b=document.createElement("b");b.textContent=name||"Viewer";const s=document.createElement("span");s.textContent=String(text).slice(0,160);d.append(b,s);list.appendChild(d);list.scrollTop=list.scrollHeight;while(list.children.length>60)list.firstChild.remove()}
...
async function sendHeart(){spawnHeart();try{await roomChannel?.send({type:"broadcast",event:"heart",payload:{sender:roomClientKey}})}catch(_){}}
function wireChat(){const input=$("#m7lv-chat-input",overlay),send=$("#m7lv-send",overlay),heart=$("#m7lv-heart-chat",overlay);const go=async()=>{const text=String(input?.value||"").trim();if(!text||!roomChannel)return;input.value="";try{await roomChannel.send({type:"broadcast",event:"chat",payload:{sender:roomClientKey,name:mode==="host"?"Shop":"Viewer",text}})}catch(_){}};if(send)send.onclick=go;if(heart)heart.onclick=sendHeart;if(input)input.addEventListener("keydown",e=>{if(e.key==="Enter"){e.preventDefault();go()}})}
