...
async function save(){
 ...
 let r=await db.from("homepage_layout_settings").update({enabled:row.enabled,position_index:row.position_index,offset_px:row.offset_px,max_width_px:row.max_width_px,updated_at:row.updated_at}).eq("key",KEY);
